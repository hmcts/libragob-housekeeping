...
if [[ $schema_id == 99 ]] || [[ $schema_id == 130 ]];then
...
#sftp -P ${sftp_port} -vvv -oStrictHostKeyChecking=no -oHostKeyAlgorithms=+ssh-rsa -i /tmp/ams-reporting/sftp-pvt-key ${sftp_username}@${sftp_endpoint} << EOF
